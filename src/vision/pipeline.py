"""
    ...
    Detection mode is controlled by use_gt:
    
      use_gt=False (default)
        MOG2 foreground detection drives the tracker.
    
      use_gt=True
        AD-SVD ground-truth bounding boxes drive the tracker.
        GT bboxes are loaded from <GT_DIR>/<VideoStem>.txt and scaled to the
        resized frame. If no GT file exists, falls back to MOG2 automatically.
        MOG2 still runs in parallel so IoU between GT and MOG2 can be shown
        in the preview and included in the comparison experiment.
...
from src.vision.detector import ForegroundDetector, load_gt_bboxes
...
def draw_frame(bgr, detections, gt_bbox, tracks,
    ...
    # Track trajectories and centroids
...
def process_video(video_path, activity="", distortion="",
                  show_preview=True, use_gt=False):
    """
    Run the full vision pipeline on one video file.
 
    Parameters
    ----------
    video_path   : path to the .mp4 file
    activity     : activity label string (for HUD display and feature labelling)
    distortion   : distortion label string
    show_preview : open an OpenCV window with live overlay
    use_gt       : if True, use AD-SVD GT bboxes to drive the tracker;
                   falls back to MOG2 if no GT file exists for this video.
                   MOG2 still runs in parallel for the IoU overlay.
 
    Returns
    -------
    tracks        : list of completed track dicts
    mean_flow_mag : float — average optical flow magnitude over the video
    mean_flow_ang : float — average optical flow angle over the video
    """
    ...
    # Read original resolution for GT coordinate scaling
    ...
    if use_gt and not using_gt:
        print(f"  [warn] No GT file for {video_path.name} — using MOG2.")
